...
def run_for_datasets(cfg_module):
    for dataset in cfg_module.DATASETS:
        cfg = cfg_module.get_config(dataset)
        params_grid = {
            "model": [dataset],
            "model.encoder": cfg["ENCODER_E2E"],
            "model.idx_flag": cfg["ANCHORS_STRATEGIES"],
            "model.n_anchors": cfg["N_CLUSTERS"],
            "model.phi_flag": ["false"],
            "model.relative": [True],
            "model.sim_type": cfg["SIMILS"],
            "seed": cfg["SEEDS"],
