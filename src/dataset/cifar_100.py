...
def get_latents(
    ...
    for batch in tqdm(dataloader, desc=f"[{split}] Computing latents"):
        with torch.no_grad():
            ...
            labels.append(batch["fine_label"].cpu())
...
def collate_fn(batch, feature_extractor, transform):
    ...
    return {
        ...
        "fine_label": torch.tensor([sample["fine_label"] for sample in batch]),
        "coarse_label": torch.tensor([sample["coarse_label"] for sample in batch]),
    }
...
if __name__ == "__main__":
    ...
    train_dataset, val_dataset = get_dataset(split="train")
